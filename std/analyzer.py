...
class Analyzer():
    ...
    def __init__(self, dir, savePath, scanid, HVPoint, scanType):
        ...
        ## lista com parametros do arquivo config já definidos no propio config para os múons
        self.muonTriggerWindow = -1    #pegando os valores da janela do múon de 5000 (ns)
        self.noiseRateTriggerWindow = -1  ##pegando os valores da janela do ruído de 5000 (ns)
        self.timeWindowReject = -1        ## pegando o valor de 100 (ns) que é rejeitado no plot  
        self.muonWindowWidth = -1    ## pegando o valor 3 para fazer a janela do múon (janela = 6*sigma)
        
        self.nStrips = -1    # pegando os valores da quantidade de strips na camara (32 strips)
        self.stripArea = -1  # pegando a area das strips 77.7 ( nao sei a dimensao da strip)
        

        ################################# Dar uma olhada dps voltando no config.py #########################################
        self.TDC_strips = [] 
        ...
        self.scanid = scanid   # pegando do config o scan id 
        self.HVPoint = HVPoint # pegando os valores de High voltage Point das camaras (cada uma tem um valor diferente KODELE e KODELH)
        self.basePath = dir    # vai pegar o diretorio que vai ser explicitado no analyzeEfficiencyRun.py por exemplo.
        self.savePath = savePath # vai salvar o caminho do diretorio.
        
        self.CMP = {} #? 
        self.CLS = {} #?
        
        if not os.path.exists(self.savePath): os.makedirs(self.savePath) # verifica se um diretório. Se não existir, ele cria o diretório.
   
        
        
        
        # trigger window
        self.scanType = scanType #?? deve passar depois esse argumento
        ...
        ####################### entender essa parte melhor provavelmente vai vir dps no código ################################
    ...
    def loadConfig(self, cfg):
        # Carregar valores do arquivo de configuração
        ...
        self.clusterSizeCut = 6 #cfg['clusterSizeCut']
        ...
        if "nStrips" in cfg:
            self.nStrips = cfg["nStrips"]
        if "stripPitch" in cfg:
            self.stripPitch = cfg["stripPitch"]
        if "stripArea" in cfg:
            self.stripArea = cfg["stripArea"]
        if "xPos" in cfg:
            self.xPos = cfg["xPos"]
        ...
        if "textHeader" in cfg:
            self.textHeader = cfg["textHeader"]
        else:
            self.textHeader = ""

        # Ajustar triggerWindow com base no tipo de scan
        if self.scanType == "efficiency":
            self.triggerWindow = self.muonTriggerWindow
        if self.scanType == "rate":
            self.triggerWindow = self.noiseRateTriggerWindow

        """
        # Imprimir valores para verificação
        print("Configurações carregadas:")
        print(f"muonTriggerWindow: {self.muonTriggerWindow}")
        print(f"noiseRateTriggerWindow: {self.noiseRateTriggerWindow}")
        print(f"timeWindowReject: {self.timeWindowReject}")
        print(f"muonWindowWidth: {self.muonWindowWidth}")
        print(f"TDC_channel_PMT: {self.TDC_channel_PMT}")
        print(f"chamberName: {self.chamberName}")
        print(f"chamberId: {self.chamberId}")
        print(f"muonClusterTimeWindow: {self.muonClusterTimeWindow}")
        print(f"muonClusterTimeWindowUp: {self.muonClusterTimeWindowUp}")
        print(f"muonClusterTimeWindowDw: {self.muonClusterTimeWindowDw}")
        print(f"gammaClusterTimeWindow: {self.gammaClusterTimeWindow}")
        print(f"gammaClusterTimeWindowUp: {self.gammaClusterTimeWindowUp}")
        print(f"gammaClusterTimeWindowDw: {self.gammaClusterTimeWindowDw}")
        print(f"clusterSizeCut: {self.clusterSizeCut}")
        print(f"TDC_strips: {self.TDC_strips}")
        print(f"TDC_strips_mask: {self.TDC_strips_mask}")
        print(f"TDC_channels: {self.TDC_channels}")
        """
        
        if hasattr(self, 'nStrips'):
            print(f"nStrips: {self.nStrips}")
        if hasattr(self, 'stripPitch'):
            print(f"stripPitch: {self.stripPitch}")
        if hasattr(self, 'stripArea'):
            print(f"stripArea: {self.stripArea}")
        if hasattr(self, 'xPos'):
            print(f"xPos: {self.xPos}")
        if hasattr(self, 'zPos'):
            print(f"zPos: {self.zPos}")

        print(f"textHeader: {self.textHeader}")
        print(f"scanType: {self.scanType}")
        print(f"triggerWindow: {self.triggerWindow}")

        # Chamar slimTree para ajustar outras configurações
        self.slimTree()
    ...
    def slimTree(self):
        ...
        fIn = ROOT.TFile("%s/Scan%.6d_HV%d_DAQ.root" % (self.basePath, self.scanid, self.HVPoint)) # lendo o arquvio ROOT de input
        """
        print(f"Tentando acessar o arquivo ROOT: {fIn}")
        # Verificando se o arquivo foi aberto corretamente
        if fIn.IsOpen():
            print(f"Arquivo {fIn} aberto.")
        else:
            print(f"Erro ao abrir o arquivo {fIn}.")
        """
        t_orig = fIn.Get("RAWData") # pegando a TTree chamada RawData com os valores que serão usados.
        """
        if t_orig:
            print("TTree 'RAWData' beleza.")
        else:
            print("Erro na TTree 'RAWData'.")

        if t_orig:
            print(f"A TTree 'RAWData' tem {t_orig.GetEntries()} entradas.")
            # Imprimindo as primeiras 5 entradas para verificação
            t_orig.Show(0)
            t_orig.Show(1)
            t_orig.Show(2)
            t_orig.Show(3)
        ...
        chs_save = copy.deepcopy(self.TDC_channels) # fazendo uma cópia de toda subestrutura (listas dentro de listas, dicionários dentro de dicionários, etc.) também serão copiadas.
        if self.TDC_channel_PMT != -1: chs_save.append(self.TDC_channel_PMT) 

        self.slimFile = "%s_HV%d.root" %(self.chamberId, self.HVPoint) # Define o nome do novo arquivo ROOT (usado para pegar todos de uma vez).
        fOut = ROOT.TFile(self.slimFile, "recreate") # arquivos ROOT de saida
        t_slim = ROOT.TTree("RAWData", "RAWData")    # abrindo a TTree do arquivo ROOT chamada RAWData (veja os arquivos)
        
        # Inicializa arrays e vetores para armazenar dados dos eventos.
        EventNumber = np.zeros(1, dtype=int)    # arrays NumPy de tamanho 1 e do tipo int.
        number_of_hits = np.zeros(1, dtype=int) # arrays NumPy de tamanho 1 e do tipo int.
        Quality_flag = np.zeros(1, dtype=int)   # arrays NumPy de tamanho 1 e do tipo int.
        TDC_channel = ROOT.vector('double')()   # vetores de double serao usados para armazenar múltiplos valores de tempo ou canal
        TDC_TimeStamp = ROOT.vector('double')() # vetores de double serao usados para armazenar múltiplos valores de tempo ou canal 
        TriggerTag = np.zeros(1, dtype=int)     # arrays NumPy de tamanho 1 e do tipo int.


        # Cria branches na TTree t_slim para armazenar os dados.
        ...
        for evNum in range(0, t_orig.GetEntries()):
            ...
            # Copia os valores dos eventos para as variáveis.####
            EventNumber[0] = evNum                              #
            ...
            #Este loop itera sobre todos os canais TDC (t_orig.TDC_channel) do evento atual. Para cada canal, 
            #verifica se ele está na lista de canais permitidos (chs_save). Se estiver, adiciona o canal (TDC_channel) 
            #e o timestamp correspondente (TDC_TimeStamp) aos vetores que serão usados para preencher a nova TTree (t_slim).
            #Isso garante que apenas os dados dos canais permitidos sejam copiados para a nova TTree.
            ...
            t_slim.Fill() # Escreve os dados do evento na nova TTree.

            TDC_channel.clear()   #Limpa os vetores para o próximo evento.
            TDC_TimeStamp.clear() #Limpa os vetores para o próximo evento.
            
        t_slim.AutoSave() # Salva automaticamente a TTree no arquivo de saída.
        fIn.Close()       # fecha o arquivo de input 
        fOut.Close()      # fecha o arquivo de output
    ...
    def timeStripProfile2D(self): 
        ...
        timeStripProfile_spill = ROOT.TH2D("timeStripProfile_spill", "", nBins, 0, 
                                           int(self.muonTriggerWindow)/1000, self.nStrips, min(self.TDC_strips), 
                                           max(self.TDC_strips)+1)
            
        nBins = 5*self.muonTriggerWindow # 500 ps resolution, scale in ns
        timeStripProfile_spill_zoom = ROOT.TH2D("timeStripProfile_spill_zoom", "", nBins, 0, 
                                                int(self.muonTriggerWindow), self.nStrips, min(self.TDC_strips), 
                                                max(self.TDC_strips)+1)  
        ...
        for i in range(1, self.nStrips+1): # isso pq a strip começa em 1 veja no config.py
    ...
    def timeProfileChannels(self, peakMean = -1, peakWidth = -1, 
                            plotSpill = True, plotNoSpill = True, xMin=-1000, xMax=1000):
    ...
    def timeProfile(self, peakMean = -1, peakWidth = -1, 
                    plotSpill = True, plotNoSpill = True):

        self.noiseGammaTimeWindowBegin = self.timeWindowReject 
        self.noiseGammaTimeWindowEnd = self.noiseRateTriggerWindow
        self.noiseGammaTimeWindow = self.noiseGammaTimeWindowEnd - self.noiseGammaTimeWindowBegin
        ...
        nBins = self.muonTriggerWindow # 1 ns resolution, scale in us
        timeProfile_spill = ROOT.TH1D("timeProfile_spill", "Time profile spill", nBins, 
                                      0, int(self.muonTimeWindowEnd)/1000)
            
        nBins = 5*self.muonTriggerWindow # 500 ps resolution, scale in ns
        timeProfile_spill_zoom = ROOT.TH1D("timeProfile_spill_zoom", "Time profile spill (zoomed)", 
                                           nBins, 0, int(self.muonTimeWindowEnd))
        
        # us resolution
        nBins = self.noiseRateTriggerWindow # 1 ns resolution, scale in us
        timeProfile_nospill = ROOT.TH1D("timeProfile_nospill", "Time profile no spill", 
                                        nBins, 0, int(self.noiseGammaTimeWindowEnd)/1000)
        ...
        if peakWidth == -1 and peakMean == -1:
            ...
            print("Dados do ajuste:")
            print(f"xMaximum: {xMaximum}, xMin_: {xMin_}, xMax_: {xMax_}")
        ...
        self.muonTimeWindowBegin = self.muonWindowMean - 37.5 #self.muonWindowWidth*self.muonWindowSigma
        self.muonTimeWindowEnd = self.muonWindowMean + 37.5 #self.muonWindowWidth*self.muonWindowSigma
        self.muonTimeWindow = self.muonTimeWindowEnd - self.muonTimeWindowBegin
        ...
        if plotSpill:
            ...
            fitParams.DrawLatex(0.16, 0.75, "#color[2]{Peak width (#sigma): %.2f ns}" % self.muonWindowSigma)
            #fitParams.DrawLatex(0.16, 0.70, "#color[2]{Muon window (2#times%d#sigma): %.2f ns}" % (self.muonWindowWidth, 6.0*self.muonWindowSigma))
            fitParams.DrawLatex(0.16, 0.70, "#color[2]{Muon window: 75 ns}") 
